...
export default function Pastor() {
 ...
 const [aceitaramJesus, setAceitaramJesus] = useState([]);
  ...
  if (tipo === "visitantes") {
    head = [["Nome", "Cargo", "Telefone", "Igreja", "Data", "Observações"]];
  ...
  if (tipo === "avisos") {
    ...
    tabela = avisos.map((a) => [a.titulo, a.descricao]);
  }

  if (tipo === "programacao") {
    head = [["Dia", "Horário", "Atividade"]];
    tabela = programacoes.map((p) => [p.dia, p.horario, p.atividade]);
  }

  if (tipo === "aceitaramJesus") {
    head = [["Nome", "Telefone", "Endereço", "Observações"]];
  ...
  autoTable(doc, {
    head,
    body: tabela,
    startY: 25, //  mais espaço do título
    styles: {
      fontSize: 10,
      cellPadding: 3, //  espaçamento interno
    },
    headStyles: {
      fillColor: [220, 38, 38],
